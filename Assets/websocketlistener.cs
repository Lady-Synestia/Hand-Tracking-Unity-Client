...
public class WebSocketListener : MonoBehaviour
{
    ...
    public event EventHandler<HandPointsChangedEventArgs>? HandPointsChanged;
    ...
    async void ReceiveData()
    {
        ...
        if (await semaphore.WaitAsync(0))
        {
            try
            {
                ...
                HandPointsChangedEventArgs args = new()
                {
                    JsonHandStrings = new string[1] { JsonString }
                };
                OnHandPointsChanged(args);
    ...
    protected virtual void OnHandPointsChanged(HandPointsChangedEventArgs e)
    {
        HandPointsChanged?.Invoke(this, e);
    }

    
}

public class HandPointsChangedEventArgs : EventArgs
{
    // Stores the strings of Json for each hand that are received over the websocket
    public string[] JsonHandStrings { get; set; }
}